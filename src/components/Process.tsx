              ...
              <div key={index} className="relative group h-full">
                ...
                {index < steps.length - 1 ? (
                  <div className="md:hidden absolute -bottom-6 left-1/2 -translate-x-1/2 z-10">
                    <div
                      className={`${step.iconBg} rounded-full p-2 shadow-lg transition-all duration-400 ${activeCard === index ? "scale-110 shadow-xl" : ""}`}
                    >
                ...
                <Card
                  ...
                  className={`relative h-full border-l-4 ${step.accent} bg-card transition-all duration-400 ${activeCard === index ? `shadow-2xl ${step.glowColor} translate-x-2` : "shadow-md hover:shadow-lg"}`}
                >
                  <CardContent className="p-8 relative h-full flex flex-col">
                    {/* Diagonal stripe decoration */}
                    <div
                      className={`absolute top-0 right-0 w-20 h-20 opacity-5 transition-opacity duration-400 ${activeCard === index ? "opacity-10" : ""}`}
                    >
                      <div
                    ...
                    <div className="flex items-start justify-between mb-6">
                      <div
                        className={`relative ${step.iconBg} w-14 h-14 rounded-lg flex items-center justify-center text-white shadow-md transition-all duration-400 ${activeCard === index ? "scale-110 rotate-6" : ""}`}
                      >
                        <step.icon className="w-7 h-7" strokeWidth={2} />

                        {/* Expanding ring */}
                        <div
                          className={`absolute inset-0 ${step.iconBg} rounded-lg transition-all duration-400 ${activeCard === index ? "scale-125 opacity-0" : "scale-100 opacity-0"}`}
                        ></div>
                      </div>

                      <div
                        className={`flex items-center justify-center w-8 h-8 rounded-full border-2 ${step.accent.replace("border-l-", "border-")} font-bold text-sm transition-all duration-400 ${activeCard === index ? "scale-125 rotate-12" : ""}`}
                      >
                        {index + 1}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mb-6 flex-grow">
                      <h3
                        className={`text-xl font-semibold transition-all duration-300 ${activeCard === index ? "translate-x-1" : ""}`}
                      >
                        {step.title}
                      </h3>
                      <p
                        className={`text-sm text-muted-foreground leading-relaxed transition-all duration-300 delay-75 ${activeCard === index ? "translate-x-1" : ""}`}
                      >
                        {step.description}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex gap-1.5 w-full">
                        {[...Array(4)].map((_, i) => (
                          <div
                            key={i}
                            className={`h-1 flex-1 rounded-full transition-all duration-300 ${activeCard === index && i < 4 ? step.iconBg : "bg-muted"}`}
                            style={{
                  ...
                  <div
                    className={`absolute bottom-0 left-0 h-0.5 ${step.iconBg} transition-all duration-500 ${activeCard === index ? "w-full" : "w-0"}`}
                  ></div>
                </Card>
              </div>
            ))}
        ...
        <div className="max-w-4xl mx-auto mt-16 flex items-center justify-center gap-2 md:gap-3">
          {steps.map((step, index) => (
            <div key={index} className="flex items-center gap-2 md:gap-3">
              <div
                className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${step.iconBg} transition-all duration-300 ${activeCard === index ? "scale-150 shadow-lg" : "scale-100"}`}
              ></div>
